...
public class AnalogClockIcon implements Icon {
	...
	private void drawHands(Graphics2D g2D, Point2D.Double center) {
		...
		double hourRotation = 30 * hour; // 360/12=30
		double minuteRotation = 6 * minute; // 360/60=6
		double secondRotation = 6 * second; //360/60=6
		
		//default hand points
		
		Point2D.Double defaultHourHand = new Point2D.Double(center.getX(), center.getY() - 30);
		Point2D.Double defaultMinuteHand = new Point2D.Double(center.getX(), center.getY() - 40);
		Point2D.Double defaultSecondHand = new Point2D.Double(center.getX(), center.getY() - 50);
		
		//rotated hand points
		
		Point2D.Double hourHand = rotatePoint(center, defaultHourHand, hourRotation);
		Point2D.Double minuteHand = rotatePoint(center, defaultMinuteHand, minuteRotation);
		Point2D.Double secondHand = rotatePoint(center, defaultSecondHand, secondRotation);
		
		g2D.setColor(Color.WHITE);
		g2D.drawLine((int) center.getX(),(int) center.getY(),(int) hourHand.getX(),(int) hourHand.getY());
		g2D.drawLine((int) center.getX(),(int) center.getY(),(int) minuteHand.getX(),(int) minuteHand.getY());
		g2D.setColor(Color.GREEN);
		g2D.drawLine((int) center.getX(),(int) center.getY(),(int) secondHand.getX(),(int) secondHand.getY());
	}
